...
import { getHistoryAction, deleteHistoryAction } from '../prediction.actions';
...
const HistoryModal: React.FC<HistoryModalProps> = ({ open, onCancel }) => {
  ...
  const history = useSelector((state: RootState) => state.prediction.history);
  const navigate = useNavigate();
  const getHistoryRef = useRef(() => {});
  getHistoryRef.current = () => {
    dispatch(getHistoryAction() as any);
  };
  useEffect(() => {
    if (open) getHistoryRef.current();
  }, [open]);
